    ...
    <AdminLayout>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input type="text" placeholder="Buscar empresa..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-input bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
        </div>
        <div className="flex gap-2">
          {["all", "pending", "approved", "active", "rejected"].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className={cn("px-3 py-2 rounded-xl text-sm transition-all whitespace-nowrap", statusFilter === status ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80")}>{status === "all" ? "Todos" : getStatusLabel(status)}</button>
          ))}
        </div>
      </div>
          ...
          <div key={quote.id} className="p-4 bg-card rounded-xl border border-border hover:bg-accent/50 transition-colors">
            ...
            {quote.status === "pending" && (
              <div className="flex gap-2 mt-4 pt-4 border-t border-border">
                <button onClick={() => updateStatus.mutate({ id: quote.id, status: "approved" })} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-success text-success-foreground text-sm hover:opacity-90"><Check className="w-4 h-4" />Aprovar</button>
                <button onClick={() => updateStatus.mutate({ id: quote.id, status: "rejected" })} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-destructive text-destructive-foreground text-sm hover:opacity-90"><X className="w-4 h-4" />Rejeitar</button>
              </div>
            )}
          </div>
        ))}
