    ...
    <AdminLayout>
      <div className="flex items-center gap-4">
        ...
        <div className="flex bg-card rounded-xl border border-border p-1">
          <button onClick={() => setViewType("retention")} className={cn("px-3 py-1.5 rounded-lg text-sm transition-all", viewType === "retention" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>Retenção</button>
          <button onClick={() => setViewType("revenue")} className={cn("px-3 py-1.5 rounded-lg text-sm transition-all", viewType === "revenue" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}>Receita</button>
        </div>
      </div>
            ...
            <thead>
              <tr className="bg-muted/50">
                ...
                {weekLabels.map((label) => (
                  <th key={label} className="text-center p-3 text-sm font-medium text-muted-foreground">{label}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={row.cohort} className="border-t border-border">
                  ...
                  {row.weeks.map((value, i) => (
                    <td key={i} className="p-1">
                      <div className={cn("p-2 text-center text-sm font-medium rounded-lg", getRetentionColor(value))}>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="p-4 bg-card rounded-xl border border-border">
          <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-success" />Tendências Positivas</h3>
        ...
        <div className="p-4 bg-card rounded-xl border border-border">
          <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2"><Calendar className="w-5 h-5 text-warning" />Pontos de Atenção</h3>
          <ul className="space-y-2 text-sm text-muted-foreground">
      ...
      <div className="p-4 bg-accent rounded-xl">
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-success" /><span className="text-muted-foreground">Excelente ({viewType === "retention" ? ">60%" : ">R$150"})</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-success/70" /><span className="text-muted-foreground">Bom ({viewType === "retention" ? "45-60%" : "R$100-150"})</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-success/50" /><span className="text-muted-foreground">Regular ({viewType === "retention" ? "30-45%" : "R$70-100"})</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-warning/50" /><span className="text-muted-foreground">Atenção ({viewType === "retention" ? "20-30%" : "R$50-70"})</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-4 rounded bg-destructive/30" /><span className="text-muted-foreground">Crítico ({viewType === "retention" ? "<20%" : "<R$50"})</span></div>
        </div>
      </div>
    </AdminLayout>
  );
