      ...
      <main className="flex-1 lg:ml-64">
        <div className="p-4 lg:p-6">
          ...
          <div className="flex gap-2 mb-6">
            <button onClick={() => setActiveTab("flags")} className={cn("px-4 py-2 rounded-lg font-medium transition-all", activeTab === "flags" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent")}>Flags ({riskFlags.length})</button>
            <button onClick={() => setActiveTab("actions")} className={cn("px-4 py-2 rounded-lg font-medium transition-all", activeTab === "actions" ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent")}>Ações ({riskActions.length})</button>
          </div>

          <div className="flex gap-3 mb-6 flex-wrap">
            <div className="flex-1 min-w-[200px] relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
              <input type="text" placeholder="Buscar usuário..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 rounded-lg border border-border bg-card text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
            {activeTab === "flags" && (
              <div className="flex gap-2">
                {(["all", "client", "pro"] as const).map(role => (
                  <button key={role} onClick={() => setRoleFilter(role)} className={cn("px-3 py-2 rounded-lg text-sm transition-all", roleFilter === role ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent")}>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={cn("px-2 py-0.5 rounded text-xs", flag.role === "client" ? "bg-primary/10 text-primary" : "bg-secondary text-secondary-foreground")}>{flag.role === "client" ? "Cliente" : "Diarista"}</span>
                      ...
                      <button onClick={() => navigate(`/admin/risk/${flag.user_id}`)} className="p-2 rounded-lg hover:bg-accent"><Eye className="w-5 h-5 text-muted-foreground" /></button>
                    </div>
                  </div>
                );
              })}
