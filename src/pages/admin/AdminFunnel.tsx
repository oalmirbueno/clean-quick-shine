          ...
          {funnelData.map((stage, i) => {
            ...
            return (
              <div key={stage.stage} className="relative">
                <div 
                  className={cn("h-16 rounded-xl flex items-center justify-between px-4 transition-all", stage.color)}
                ...
                >
                  ...
                  <div className="text-right text-white">
                    ...
                    {i > 0 && <p className="text-xs opacity-80">{stage.rate}% conversão</p>}
                  </div>
                </div>
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          ...
          <div className="space-y-4">
            {conversionMetrics.map((metric) => (
              <div key={metric.from} className="flex items-center justify-between p-3 bg-accent rounded-xl">
                <span className="text-sm text-foreground">{metric.from}</span>
                <div className="flex items-center gap-3">
                  <span className="font-bold text-foreground">{metric.rate}%</span>
                  <span className={cn("text-xs px-2 py-0.5 rounded", metric.change > 0 ? "bg-success/20 text-success" : "bg-destructive/20 text-destructive")}>
                  ...
                  {stage.reasons.map((reason) => (
                    <div key={reason.reason}>
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-destructive/60 rounded-full" style={{ width: `${reason.percentage}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
