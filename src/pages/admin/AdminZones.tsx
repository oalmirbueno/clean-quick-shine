  ...
  return (
    <AdminLayout>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {cities.map((city: any) => (
          <button key={city.id} onClick={() => setSelectedCity(city.id)} className={cn("px-4 py-2 rounded-full whitespace-nowrap transition-all text-sm", activeCityId === city.id ? "bg-primary text-primary-foreground" : "bg-card border border-border text-foreground hover:bg-accent")}>
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" placeholder="Buscar zona..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-input bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary" />
      </div>
