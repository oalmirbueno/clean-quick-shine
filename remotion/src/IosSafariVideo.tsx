...
const Tap: React.FC<{ x: number; y: number; delay?: number; size?: number }> = ({
  ...
  return (
    <div style={{ position: "absolute", left: x - size / 2, top: y - size / 2, width: size, height: size, pointerEvents: "none" }}>
      <div style={{ position: "absolute", inset: 0, borderRadius: "50%", border: `3px solid ${MINT}`, transform: `scale(${ring})`, opacity: ringOp }} />
      <div style={{ position: "absolute", inset: size * 0.28, borderRadius: "50%", background: MINT, boxShadow: `0 0 30px ${MINT}`, transform: `scale(${dot})` }} />
    </div>
  );
};

const Callout: React.FC<{ x: number; y: number; text: string; side?: "left" | "right" | "top" | "bottom"; delay?: number }> = ({
  ...
  return (
    ...
    >
      <div style={{ background: MINT, color: "#062017", fontFamily: FF, fontWeight: 700, fontSize: 22, padding: "12px 18px", borderRadius: 14, whiteSpace: "nowrap", boxShadow: `0 10px 30px ${MINT}66` }}>
        {text}
      </div>
    </div>
  );
...
const IPhone: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div style={{ width: IPHONE_W, height: IPHONE_H, borderRadius: 72, background: "#0A0A0A", padding: 14, boxShadow: "0 50px 120px rgba(0,0,0,0.65), 0 0 0 2px rgba(255,255,255,0.08) inset", position: "relative" }}>
    <div style={{ width: "100%", height: "100%", borderRadius: 58, background: "black", overflow: "hidden", position: "relative" }}>
      <div style={{ position: "absolute", top: 14, left: "50%", transform: "translateX(-50%)", width: 130, height: 34, background: "#0A0A0A", borderRadius: 20, zIndex: 30 }} />
      {children}
    </div>
  </div>
);

// Safari page — the top-of-page screenshot fills the phone; Safari bottom bar
// (URL + toolbar) is baked in at the bottom of the screenshot.
const SafariPageBg: React.FC = () => (
  <Img
...
// In Safari iOS the Share icon sits in the BOTTOM toolbar, roughly centered.
// Screenshot ~810x1620 => bottom bar around y≈1560, share center ≈ x=405
// Ratios: x=0.5, y=0.96
const SHARE_X = IPHONE_W * 0.5;
const SHARE_Y = IPHONE_H * 0.955;
  ...
  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", fontFamily: FF }}>
      ...
      <div style={{ textAlign: "center", opacity: s }}>
        ...
        <div style={{ marginTop: 60, color: MINT_SOFT, fontSize: 26, fontWeight: 600, letterSpacing: 6, textTransform: "uppercase", opacity: s2 }}>
          Instalar no iPhone
        </div>
        <div style={{ marginTop: 18, color: "white", fontSize: 78, fontWeight: 800, lineHeight: 1.05, opacity: s2, transform: `translateY(${(1 - s2) * 20}px)` }}>
          Safari, em<br />4 passos.
        </div>
      </div>
    </AbsoluteFill>
  );
...
// Step wrapper
// ============================================================
const StepShell: React.FC<{ n: number; title: string; subtitle?: string; children: React.ReactNode }> = ({
  ...
  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "flex-start", fontFamily: FF, paddingTop: 90 }}>
      <Bg />
      <div style={{ display: "flex", alignItems: "center", gap: 20, transform: `translateY(${(1 - s) * -20}px)`, opacity: s }}>
        <div style={{ width: 72, height: 72, borderRadius: 22, background: MINT, color: "#062017", display: "grid", placeItems: "center", fontSize: 34, fontWeight: 800, boxShadow: `0 0 30px ${MINT}77` }}>
          {n}
        </div>
...
// ============================================================
// Step 1 — jalimpo.com aberto no Safari
// ============================================================
const Step1: React.FC = () => (
  <StepShell n={1} title="Você está em jalimpo.com" subtitle="Aberto no Safari do seu iPhone">
    <div style={{ position: "relative" }}>
      <IPhone><SafariPageBg /></IPhone>
    </div>
  </StepShell>
);

// ============================================================
// Step 2 — toque em Compartilhar (barra inferior do Safari)
// ============================================================
const Step2: React.FC = () => {
  const frame = useCurrentFrame();
  return (
    <StepShell n={2} title="Toque em Compartilhar" subtitle="Ícone na barra inferior do Safari">
      <div style={{ position: "relative" }}>
        <IPhone>
          <SafariPageBg />
          {/* Highlight ring around the share button in the Safari bottom bar */}
          <div
            style={{
              position: "absolute",
              left: SHARE_X - 44,
        ...
        <Tap x={SHARE_X} y={SHARE_Y} delay={12} size={110} />
        <Callout x={SHARE_X} y={SHARE_Y - 90} text="Toque no ícone Compartilhar" side="top" delay={20} />
      </div>
    </StepShell>
  );
};
    ...
    <StepShell n={3} title='Escolha "Adicionar à Tela de Início"'>
      <div style={{ position: "relative" }}>
        <IPhone>
          ...
          <div style={{ position: "absolute", left: 10, right: 10, bottom: 24, width: sheetWidth, transform: `translateY(${y}px)`, fontFamily: FF }}>
            <div style={{ background: "rgba(245,245,247,0.98)", borderRadius: 22, padding: 16, display: "flex", alignItems: "center", gap: 14, marginBottom: 10, boxShadow: "0 12px 40px rgba(0,0,0,0.4)" }}>
              <div style={{ width: 54, height: 54, borderRadius: 14, background: MINT, display: "grid", placeItems: "center" }}>
            ...
            <div style={{ background: "rgba(245,245,247,0.98)", borderRadius: 20, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,0.4)" }}>
              {[
...
// ============================================================
// Step 4 — Confirmar "Adicionar" (canto superior direito)
// ============================================================
        ...
        <IPhone>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <div style={{ background: MINT, color: "white", padding: "8px 18px", borderRadius: 20, fontWeight: 800, fontSize: 17, boxShadow: `0 0 24px ${MINT}` }}>
                Adicionar
              </div>
            </div>
            <div style={{ background: "white", borderRadius: 16, padding: 18, display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ width: 68, height: 68, borderRadius: 16, background: MINT, display: "grid", placeItems: "center" }}>
                <Img src={staticFile("images/logo-icon.png")} style={{ width: "70%", height: "70%", objectFit: "contain" }} />
              </div>
              <div>
...
// ============================================================
// Step 5 — Home screen com ícone aparecendo
// ============================================================
const HomeScreen: React.FC = () => {
  ...
  const apps = [
    ...
    { label: "Safari", color: "#FFFFFF", letter: "S", dark: true },
    { label: "WhatsApp", color: "#25D366", letter: "W" },
  ];

  return (
    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(160deg, #1F2A44 0%, #3A2E5A 60%, #5A3E68 100%)", padding: "80px 32px 40px", fontFamily: FF }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 22 }}>
        {apps.map((a, i) => (
          <div key={i} style={{ textAlign: "center" }}>
            <div style={{ width: 94, height: 94, borderRadius: 22, background: a.color, display: "grid", placeItems: "center", fontSize: 42, fontWeight: 800, color: a.dark ? "#0A0A0A" : "white", boxShadow: "0 8px 18px rgba(0,0,0,0.3)" }}>
              {a.letter}
            </div>
        ...
        <div style={{ textAlign: "center", transform: `scale(${pop})` }}>
          <div style={{ width: 94, height: 94, borderRadius: 22, background: MINT, display: "grid", placeItems: "center", boxShadow: `0 8px 30px ${MINT}, 0 0 ${60 * glow}px ${MINT}` }}>
            <Img src={staticFile("images/logo-icon.png")} style={{ width: "70%", height: "70%", objectFit: "contain" }} />
          </div>
          <div style={{ color: "white", fontSize: 14, marginTop: 8, fontWeight: 800, textShadow: "0 1px 3px rgba(0,0,0,0.7)" }}>Já Limpo</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <AbsoluteFill style={{ alignItems: "center", justifyContent: "center", fontFamily: FF }}>
      ...
      <div style={{ textAlign: "center", opacity: s }}>
        ...
        <div style={{ marginTop: 60, color: "white", fontSize: 84, fontWeight: 800, lineHeight: 1.02, opacity: s2, transform: `translateY(${(1 - s2) * 20}px)` }}>
          Chamou,<br />tá limpo.
        </div>
        <div style={{ marginTop: 28, color: MINT_SOFT, fontSize: 26, fontWeight: 600, letterSpacing: 4, textTransform: "uppercase", opacity: s2 }}>
          jalimpo.com
        </div>
      </div>
    </AbsoluteFill>
...
const S: React.FC<{ from: number; d: number; children: React.ReactNode }> = ({ from, d, children }) => (
  <Sequence from={from} durationInFrames={d}>{children}</Sequence>
);

export const IosSafariVideo: React.FC = () => {
  let t = 0;
  const push = (d: number, node: React.ReactNode, key: string) => {
    const el = (<S key={key} from={t} d={d}>{node}</S>);
    t += d;
    return el;
  };
  const els = [
